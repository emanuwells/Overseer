...
import type { DatabaseInfo, OverviewData, Pipeline, Run, RunDetail } from '../lib/types';
import {
  criticalityClass,
  dedupePipelines,
...
export function OperationsPage() {
  ...
  const runDetailQuery = useQuery({
    queryKey: ['run-detail', modalRunId],
    enabled: Boolean(modalRunId),
    queryFn: () => api<RunDetail>(`/v1/read/runs/${encodeURIComponent(modalRunId!)}`),
  });
  ...
  const inspectorItem = selected || filtered[0] || null;

  const openRow = (item: Pipeline) => {
    setSelected(item);
    setDrawerOpen(true);
  };
  ...
  return (
    ...
    >
      <p className="mb-4 text-sm text-slate-400">
        Estado por deployment (pipeline + host). Clique numa linha para inspeccionar. Interface read-only.
      </p>
      <TokenBanner />
            ...
            <div className="mt-2 flex items-baseline gap-2">
              {kpi.to ? (
                ...
                <button
                  type="button"
                  onClick={kpi.onClick}
                  className={`font-mono text-2xl font-semibold hover:text-blue-300 ${kpi.danger ? 'text-rose-400' : ''}`}
                >
                  {kpi.value}
                </button>
              ) : (
      ...
      <div className="mt-4 hidden xl:block">
        <aside className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
          <PipelineInspector
            ...
            onRunClick={(run: Run) => setModalRunId(run.run_id)}
          />
        </aside>
      </div>
      ...
      <Drawer
        open={drawerOpen}
        title={inspectorItem ? pipelineLabel(inspectorItem).title : 'Deployment'}
      ...
      >
        {runDetailQuery.data?.run ? (
          <RunDetailPanel
            run={runDetailQuery.data.run}
            modules={runDetailQuery.data.modules || []}
            logs={runDetailQuery.data.logs || []}
            pipelines={pipelines}
          />
        ) : runDetailQuery.isLoading ? (
          <p className="text-sm text-slate-400">A carregar…</p>
        ) : (
          <p className="text-sm text-slate-400">Run não encontrada.</p>
        )}
      </Modal>
    </AppShell>
  );
