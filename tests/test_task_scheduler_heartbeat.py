    ...
    heartbeat = store.record_heartbeat(
        {
            ...
            "payload": {
                "task_scheduler": {
                    "ok": True,
                    "pipelines": [{"pipeline_id": "medidata_pipeline", "task_found": True}],
                }
            },
        }
    )
...
def test_read_heartbeats_returns_task_scheduler_payload(sqlite_store) -> None:
    store.record_heartbeat(
        {
            ...
            "payload": {
                "task_scheduler": {
                    "ok": True,
                    "pipelines": [{"pipeline_id": "medidata_pipeline", "next_run_time": "2026-06-17T07:30:00Z"}],
                }
            },
        }
    )
    ...
    assert item["payload"]["task_scheduler"]["pipelines"][0]["pipeline_id"] == "medidata_pipeline"
