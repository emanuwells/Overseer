...
def test_orchestrate_trigger_dispatches_via_ssh(
    ...
    mock_dispatch.return_value = {"ok": True, "host": "linux-host", "mode": "local", "exit_code": 0}
...
def test_orchestrate_trigger_rejects_suspended(mock_find, _mock_ssh_enabled):
    ...
    response = client().post(
        "/v1/orchestrate/triggers",
        json={"pipeline_id": "example_pipeline", "host_id": "windows-host"},
    )
...
def test_orchestrate_trigger_requires_ssh(_mock_ssh_enabled):
    response = client().post(
        "/v1/orchestrate/triggers",
        json={"pipeline_id": "demo", "host_id": "LINUX-HOST"},
    )
...
def test_execute_pipeline_run_local(mock_local, tmp_path, monkeypatch):
    ...
    result = runner_ssh.execute_pipeline_run("linux-host", "scheduled_pipeline", requested_by="tester", root=tmp_path)
    assert result["ok"] is True
    assert result["mode"] == "local"
    assert "scheduled_pipeline" in mock_local.call_args[0][0]
...
def test_orchestrate_trigger_fails_when_dispatch_fails(
    ...
    mock_find.return_value = {"pipeline_id": "demo", "host_id": "windows-host", "metadata": {}}
    ...
    response = client().post(
        "/v1/orchestrate/triggers",
        json={"pipeline_id": "demo", "host_id": "windows-host"},
    )
