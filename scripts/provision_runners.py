...
``host_id`` da API — o ``pipeline_id`` mantém-se lógico (ex. ``example_pipeline``).
"""
...
def runners_catalog_dir(repo_root: pathlib.Path | None = None) -> pathlib.Path:
    configured = os.environ.get("OVERSEER_RUNNERS_DIR", "").strip()
    if configured:
...
def resolve_runner_catalog(
    *,
    ...
    """Resolve o catálogo privado correspondente ao host atual."""
    ...
    raise FileNotFoundError(
        ...
        f"Tentado: {tried}. Disponíveis em {catalog_dir}: {available_text}. "
        "Cria um catálogo no diretório configurado ou define OVERSEER_RUNNERS_CATALOG."
    )
...
def main() -> int:
    ...
    for item in pipelines:
        ...
        if platform == "windows":
            ...
            entry["task_name"] = item.get("task_scheduler_name") or f"Overseer - {logical_id}"
