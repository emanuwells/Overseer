...
def migrate(*, baze2_host: str = DEFAULT_BAZE2_HOST, dry_run: bool = False) -> int:
    store.init_schema()
    ...
    with engine.connect() as conn:
        ...
        for row in rows:
            ...
            elif not current_host and baze2_host:
                changes.append((pid, pid, baze2_host))
...
def main() -> int:
    ...
    parser.add_argument("--baze2-host", default=DEFAULT_BAZE2_HOST, help="host_id para pipelines sem sufixo.")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    return migrate(baze2_host=args.baze2_host, dry_run=args.dry_run)
