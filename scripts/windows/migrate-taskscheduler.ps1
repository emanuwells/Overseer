...
Write-Host ""
Write-Host "Substituídas $($replaced.Count) tarefas: $([string]::Join(', ', ($replaced | Select-Object -Unique)))"
if ($missing) {
    Write-Warning "Sem correspondência no Task Scheduler: $([string]::Join(', ', ($missing | Select-Object -Unique)))"
}
