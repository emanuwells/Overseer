...
from .runner_catalog import runners_dir
...
def resolve_catalog_host_id(host_id: str, root: Path | None = None) -> str:
    """Mapeia um host_id para o identificador canónico configurado."""
...
def build_sync_command(
    ...
    *,
    ...
    catalog_path = str(cfg.get("catalog_path") or f"deploy/runners/{host_id}.yaml").strip()
    encoded = base64.b64encode(catalog_payload).decode("ascii") if catalog_payload else ""

    if platform == "windows":
        repo_win = repo_path.rstrip("\\")
        prefix = ""
        if encoded:
            catalog_win = f"%TEMP%\\overseer-{host_id}.yaml"
            prefix = (
                "powershell -NoProfile -Command "
                f"\"[IO.File]::WriteAllBytes('$env:TEMP\\overseer-{host_id}.yaml',"
                f"[Convert]::FromBase64String('{encoded}'))\" && "
            )
        elif Path(catalog_path).is_absolute() or ":" in catalog_path:
            catalog_win = catalog_path
        else:
            catalog_win = f"{repo_win}\\{catalog_path.replace('/', chr(92))}"
        commands = [
            f'{prefix}cd /d "{repo_win}" && git pull',
            (
    ...
    prefix = ""
    if encoded:
        catalog_path = f"/tmp/overseer-{host_id}.yaml"
        prefix = f"printf %s {shlex.quote(encoded)} | base64 -d > {shlex.quote(catalog_path)} && "
    prov = f"bash scripts/provision-runners.sh --register --catalog={shlex.quote(catalog_path)}"
    if schedule_changed:
        prov += " --crontab"
    return f"{prefix}cd {repo_q} && git pull && {prov}"
...
def sync_remote_runner(
    ...
    *,
    ...
    local_catalog = runner_catalog.catalog_path_for_host(canonical_host, root)
    local_target = is_local_ssh_target(ssh_target)
    if local_target:
        cfg = {**cfg, "catalog_path": str(local_catalog)}
